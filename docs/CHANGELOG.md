...
- Enhance `vminfo ps` on Linux with positional and `--filter` process search, `--limit`, text `--tree`, `--watch` with `--count` / `--interval`, `AGE` / `COMMAND` columns, and JSON `command` / `started_at_unix` process fields.
...
- Add `docs/api.md` and `docs/feature-benchmark.md` to document the HTTP API, competitor feature benchmarking, and a prioritized product roadmap for `vminfo`.
